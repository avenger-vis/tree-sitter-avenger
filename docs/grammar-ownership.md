...
| `sql_expr` | `avenger_sql` | inherited `_expression` under one of three expression wrappers |
...
| EBNF production | Combined grammar rule/node |
| --- | --- |
| `slot`, `slot_shape` | `slot_declaration`, contextual shape child |
| `channel_param` | `channel_parameter_declaration` |
| `output` | `output_declaration` with `sql_terminated_expression` |
| `export` | `export_declaration` |
| `match_block`, `match_arm` | `match_declaration`, `match_arm` |
| `splice` | `block_splice` |
| `param` | `param_declaration` with specialized param body |
| `store` | `store_declaration` |
| `selection` | `selection_declaration` |
...
| `overlay` | `overlay_declaration` |
| `layer` | `layer_declaration` |
| `when` | `when_declaration` |
| `field` | `field_declaration` with structural `arrow_type` |
| `row` | `row_declaration` |
| `key` | `key_declaration` |
...
| `equality_predicate` | `equality_declaration` |
| `interval_predicate` | `interval_declaration` |
...
| `action` | `set_action` |
| `state_action` | `set_action` with `target:` and `value:` fields |
| `cursor_action` | cursor alternative of `set_action` with `value:` only |
...
| structurally terminated header expression | `sql_terminated_expression` |
...
It is reachable only from param `type:` specialization and `field_declaration`;
ordinary properties named `type` remain ordinary expressions.
